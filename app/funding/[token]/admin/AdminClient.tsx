...
export default function AdminClient({ funding, payments, totalAmount, goalReached }: Props) {
  const router = useRouter()
  const [isPending, startTransition] = useTransition()
  const [showDeleteModal, setShowDeleteModal] = useState(false)

  const displayPayments = payments.length > 0 ? payments : MOCK_PAYMENTS
  const displayTotal = payments.length > 0 ? totalAmount : MOCK_PAYMENTS.reduce((s, p) => s + p.amount, 0)
    ...
    <>
      ...
      <main className="px-4 py-6 flex flex-col gap-5 pb-10">
        ...
        <div className="bg-rose-50 rounded-2xl p-5">
          ...
          <p className="text-3xl font-bold text-rose-500">{formatKRW(displayTotal)}</p>
          <p className="text-xs text-gray-400 mt-1">{displayPayments.length}명 참여</p>
        </div>

        {/* 수정 / 삭제 */}
        <div className="flex gap-3">
          <Link href={`/funding/${funding.share_token}/edit`} className="flex-1">
        ...
        <div className="flex flex-col gap-2">
          <h2 className="text-sm font-semibold text-gray-700">
            펀딩 참여자 <span className="text-gray-400 font-normal">{displayPayments.length}명</span>
          </h2>
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  {p.message && (
                    <p className="text-xs text-gray-400 mt-0.5 truncate">&ldquo;{p.message}&rdquo;</p>
                  )}
                </div>
              </div>
              <span className="text-sm font-bold text-rose-500 shrink-0">{formatKRW(p.amount)}</span>
            </div>
          ))}
        </div>
